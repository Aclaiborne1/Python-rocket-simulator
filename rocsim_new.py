...
def user_input():
	...
	display_engines()
	while True:
		opt=raw_input("(U)pdate engine data, (R)un new simulation or (Q)uit: ")
		if (opt in ['U','u']):
			return 'update'
			break
		if(opt in ['R','r']):
...
def update_engine():
	...
	print "***********************************************************************************************************"
	print
	engine=raw_input("Enter engine type:")
	...
	while True:
		thrust_list = raw_input("Enter list in format '[val1, val2,...,valn]': ")
	...
	while True:
		opt=raw_input("(N)ewtons or (P)ounds?")
		if (opt in ['N','n','P','p']):
			break
		else:
...
#
	while(Velocity >= 0):
		...
		Time=interval * 0.1
		print '%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f'%(Time, Altitude, Velocity, Acceleration, Gees, Total_mass_kg*1000.0)
		interval +=1
